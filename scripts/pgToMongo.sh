...
migrate_table() {
    ...
    if [[ ! -z $mongo_index_cmd ]]
    then
        docker exec $MONGO_CONTAINER_NAME mongosh mongodb://$MONGO_USER:$MONGO_PASSWORD@$MONGO_HOST:$MONGO_PORT/$MONGO_DB --authenticationDatabase admin --eval "$mongo_index_cmd" &> /dev/null
        echo "Created index for $collection_name in Mongo"
    fi
...
MONGO_E01_TELEFONO_INDEX_CMD="\"db.E01_TELEFONO.createIndex( { codigo_area:1, nro_telefono:1 }, { unique:true } )\""
migrate_table "$PG_COPY_E01_TELEFONO" "$MONGO_E01_TELEFONO_COLLECTION_NAME" "$MONGO_E01_TELEFONO_PARAMS" "$MONGO_E01_TELEFONO_INDEX_CMD"
...
MONGO_E01_FACTURA_INDEX_CMD="db.E01_FACTURA.createIndex( { _id:1, 'detalles_factura.nro_item':1 }, { unique:true } )"
migrate_table "$PG_COPY_E01_FACTURA" "$MONGO_E01_FACTURA_COLLECTION_NAME" "$MONGO_E01_FACTURA_PARAMS" "$MONGO_E01_FACTURA_INDEX_CMD"
